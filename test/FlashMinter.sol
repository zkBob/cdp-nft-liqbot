...
import "@cdp/lib/openzeppelin-contracts/contracts/security/ReentrancyGuard.sol";
import "@cdp/lib/openzeppelin-contracts/contracts/token/ERC20/IERC20.sol";
...
contract FlashMinter is IERC3156FlashLender, ReentrancyGuard, Ownable {
    ...
    function flashLoan(
        ...
        IMintableBurnableERC20(_token).mint(address(_receiver), _amount);
        require(
            _receiver.onFlashLoan(msg.sender, _token, _amount, fee, _data) == _RETURN_VALUE,
            "FlashMinter: invalid return value"
        );
        IERC20(_token).transferFrom(address(_receiver), address(this), _amount);
